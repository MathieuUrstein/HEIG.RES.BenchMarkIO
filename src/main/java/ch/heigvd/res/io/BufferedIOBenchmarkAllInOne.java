...
public class BufferedIOBenchmarkAllInOne {
	...
	private void produceTestData(IOStrategy ioStrategy, long numberOfBytesToWrite, int blockSize) {
		LOG.log(Level.INFO, "Generating test data ({0}, {1} bytes, block size: {2}...",
            new Object[]{
                ioStrategy,
        ...
        try {
            metrics.write(("WRITE,"+ioStrategy+","+blockSize+","+numberOfBytesToWrite+","+String.valueOf(time)+"\n").getBytes());
        } catch (IOException e) {
            e.printStackTrace();
        }
	...
	private void consumeTestData(IOStrategy ioStrategy, int blockSize) {
        ...
        long time = Timer.takeTime();
        try {
            metrics.write(("READ,"+ioStrategy+","+blockSize+","+totalBytes+","+String.valueOf(time)+"\n").getBytes());
        } catch (IOException e) {
            e.printStackTrace();
        }
        LOG.log(Level.INFO, "  > Done in {0} ms.", time);
	...
	public static void main(String[] args) {
        try {
            for (int i = 0; i < NB_METRICS; i++) {
                metrics = new FileOutputStream("metricsAnalysis-"+i+".csv");
                metrics.write("operation,strategy,blockSize,fileSizeInBytes,durationInMs\n".getBytes());

                System.setProperty("java.util.logging.SimpleFormatter.format", "%5$s %n");

                BufferedIOBenchmarkAllInOne bm = new BufferedIOBenchmarkAllInOne();
